...
import { AddressLike, BaseContract, BigNumberish, BytesLike, Signer, TransactionRequest } from 'ethers'
import { UserOperationStruct } from '~/contracts/contracts/ChainlinkAccount'
import { ChainlinkAccount, ChainlinkAccountFactory, EntryPoint } from '../contracts'
...
export const makeUserOpBuilder = (entryPoint: EntryPoint, factory: ChainlinkAccountFactory, chainId: BigNumberish) => {
  const makeNonce = async (request: TransactionRequest, account: ChainlinkAccount) =>
    request.nonce ?? entryPoint.getNonce(account.getAddress(), DEFAULT_KEY)

  const makeInitCode = (request: TransactionRequest, owner: AddressLike, account: ChainlinkAccount) =>
      ...
      ? entryPoint.runner.provider
          ...
          .then(async (code) =>
            code.length > 2
              ? '0x'
              : ethers.concat([
                  await (factory as BaseContract).getAddress().then((address) => address as BytesLike),
                  factory.interface.encodeFunctionData('createAccount', [owner, DEFAULT_SALT]),
                ])
          )
      : '0x'

  const makeCallData = async (request: TransactionRequest, account: ChainlinkAccount) => {
    if (!request.to) {
  ...
  const makeCallGasLimit = (request: TransactionRequest) =>
    ...
    DEFAULT_CALL_GAS
  const makeVerificationGasLimit = (request: TransactionRequest) => DEFAULT_VERIFICATION_GAS_LIMIT // TOOD
  const makePreVerificationGas = (request: TransactionRequest) => DEFAULT_PREVERIFICATION_GAS // TOOD
  const makeMaxFeePerGas = (request: TransactionRequest) =>
    request.maxFeePerGas ??
    entryPoint.runner?.provider
      ?.getBlock('latest')
      .then((b) => (b && b.baseFeePerGas ? b.baseFeePerGas * 3n : DEFAULT_MAX_FEE_PER_GAS)) ??
    DEFAULT_MAX_FEE_PER_GAS
  const makeMaxPriorityFeePerGas = (request: TransactionRequest) =>
    request.maxPriorityFeePerGas ??
    entryPoint.runner?.provider
      ?.getBlock('latest')
      .then((b) => (b && b.baseFeePerGas ? b.baseFeePerGas : DEFAULT_MAX_PRIORITY_FEE)) ??
...
export const makeSignPayload =
  (chainId: BigNumberish, eip712Name: string = 'ChainlinkAccount', version: string = '1') =>
  async (account: ChainlinkAccount, userOpData: Omit<UserOperationStruct, 'signature'>) => {
    const domainData = {
      name: eip712Name,
      version,
      chainId,
    ...
    return ethers.TypedDataEncoder.getPayload(domainData, { UserOp }, userOpData)
  }
